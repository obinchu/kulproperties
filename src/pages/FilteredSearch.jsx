...
propertyDetails[0].properties.forEach((property) => {
  ...
  const validstatuses =   [...statuses]
        const statusIsValid = (status) => {
            // Example: Check if status exists in a list of valid statuses
            // const validstatuses = ["status1", "status2", "status3"];
            return validstatuses.includes(status);
          };
        // Check if the status is valid (you can implement your own validation logic here)
        const isValidstatus = statusIsValid(status);
      
        if (!isValidstatus) {
          // Redirect or render the Error component if the status is not valid
          return <Error />;
        }
  return (
