...
const PropertyCards = () => {
    ...
    const HouseDetails = [
      ...
      return (
        <div className='w-full h-full md:h-[85%] flex flex-col rounded text-sm text-primary'>
          <div className='flex relative w-full max-w-6xl m-auto h-full rounded'>
            <div className='flex w-full h-full overflow-x-scroll no-scrollbar rounded p-1'>
              {HouseDetails.map((details, index) => (
                ...
                >
                  <div className="flex flex-col p-2 w-full h-full bg-black/20 justify-between">
                    <span className='rounded p-1 w-[23%] text-base justify-center bg-red-500 text-white flex'>{details.Status}</span>
                    <span className=' text-other font-medium text-xl'>KES {details.Price}</span>
                    </div>  
                </div>
